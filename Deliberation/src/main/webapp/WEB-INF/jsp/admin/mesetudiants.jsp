...
<layout:extends name="layout.jsp">
	<layout:put block="content" type="REPLACE">
		...
		<div class="main-card mb-3 card">
			<div class="card-body">
				<h5 class="card-title">Resultat de deliberation</h5>
				<form class="form-group"
					action="/professeur/listerElements/${element.id_element}"
					method="POST">
					<div class="col-md-6">
						<div class="position-relative form-group">
							<label for="name" class="">Deliberation</label> <select
								name="annee" id="exampleSelect" class="form-control">
								<c:forEach var="annee" items="${e}">
				...
				<table class="mb-0 table table-striped" id="myTable">
					<thead>
						<tr>
							<th class="th-sm">Etudiant</th>
							<c:forEach var="module" items="${structures.keySet()}">
								<th class="th-sm">${module.libelle_module}</th>
							</c:forEach>
						</tr>
					</thead>
					<tbody id="etudiants">
						<c:forEach var="etudiant"
							items="${structures.entrySet().keySet()}">
							<tr>
								<td>${etudiant.last_name_fr etudiant.first_name_fr}</td>
								<c:forEach var="module" items="${structures.keySet()}">
									<td><a style="color: black" href="#">${structures.get(module).get(etudiant)}</a></td>
								</c:forEach>
							</tr>
						</c:forEach>
					</tbody>
