...
public class PDFExport {
	...
	public void generatePvElement(List<NoteElement> notesElement, Element element)
			throws DocumentException, MalformedURLException, IOException {
		document.addTitle("Deliberation pour " + element.getLibelle_element());
		Image img = Image.getInstance(BACHELOR_LINK_LOGO);
		document.add(img);
		Font textDeValeur = new Font(Font.COURIER, 18, Font.BOLD);
		textDeValeur.setColor(Color.RED);
		...
		Phrase phrase1 = new Phrase("Element :");
		phrase1.setFont(textDeValeur);

		Phrase phrase2 = new Phrase(element.getLibelle_element());
		phrase2.setFont(simpleText);
		...
		Paragraph paragReponsable = new Paragraph();
		paragReponsable.setSpacingBefore(14);
		Phrase phrase11 = new Phrase("Professeur :");
		phrase11.setFont(textDeValeur);

		Phrase phrase22 = new Phrase(element.getProfesseurs().get(0).getNom_professeur() + " "
				+ element.getProfesseurs().get(0).getPrenom_professeur());
		phrase22.setFont(simpleText);
		 ...
		 * Remplir tableau
		 */
		for (NoteElement note : notesElement) {
			PdfPCell massarCell1 = new PdfPCell();
			Phrase massarPhrase1 = new Phrase(note.getIdCompose().getEtudiant().getMassar_edu());
			massarPhrase1.setFont(simpleText);
			massarCell1.setPhrase(massarPhrase1);
			...
			if (!note.isAbscent()) {
				...
				PdfPCell noteCell1 = new PdfPCell();
				Phrase notePhrase1 = new Phrase("Abscent");
				notePhrase1.setFont(simpleText);
				noteCell1.setPhrase(notePhrase1);
				noteCell1.setBackgroundColor(c);
				noteCell1.setPadding(5);
	...
	public void generateUltimatePv(Semestre semestre, NotesModuleRepository noteModuleRepository,
		...
		for (NoteSemestre note : deliberation.getNotesSemestre()) {
			...
			for (Modulee module : semestre.getModules()) {
				NoteModule noteModule = noteModuleRepository.getOne(new ComposedNoteModule(module, etudiant,deliberation.getAnneeAcademique()));
				Color c = null;
	...
	public void generateEtudiantReleve(Semestre semestre, Etudiant etudiant, NotesModuleRepository noteModuleRepository,AnneeAcademique annee)
			throws DocumentException, MalformedURLException, IOException {
		...
		for (Modulee module : semestre.getModules()) {
			...
			try {
				NoteModule note = noteModuleRepository.getOne(new ComposedNoteModule(module, etudiant,annee));
	...
	public void generateScolarCertificat(Semestre semestre, Etudiant etudiant)
			throws DocumentException, MalformedURLException, IOException {
