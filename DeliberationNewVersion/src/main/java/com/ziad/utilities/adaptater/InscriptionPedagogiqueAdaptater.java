...
public class InscriptionPedagogiqueAdaptater implements JsonSerializer<InscriptionPedagogique>{
	...
	public JsonElement serialize(InscriptionPedagogique src, Type typeOfSrc, JsonSerializationContext context) {
		...
		json_object.addProperty("id_annee_academique", src.getAnnee_academique().getId_annee_academique());
		json_object.addProperty("massar", src.getId_inscription_pedagogique().getEtudiant().getMassar_edu());
		...
		json_object.addProperty("annee", src.getAnnee_academique().getAnnee_academique());
		return json_object;
	}
