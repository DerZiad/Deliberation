...
<layout:extends name="../layout.jsp">
	<layout:put block="content" type="REPLACE">
		...
		<div class="main-card mb-3 card">
			<div class="card-body">
				...
				<form method="POST" action="/inscription/ModifierAnneeDiplomante">
					...
					<c:forEach var="filiere" items="${fileres}">
							...
							<table class="mb-0 table table-hover">
								...
								<tbody>
									<c:forEach var="etape" items="${etapes}">
										<tr>
											<c:if test="${filiere.id_filiere==etape.filiere.id_filiere }">
												<td><a style="color: black">${etape.libelle_etape}</a></td>
												<td><a style="color: black">${etape.filiere.nom_filiere }</a></td>
												<c:forEach var="semestre" items="${semestres }">
													<c:if test="${semestre.etape== etape}">
														<td><a style="color: black" href="#linktobottom"
															onclick="tableauModule(${semestre.id_semestre})">${semestre.libelle_semestre}</a></td>
													</c:if>
												</c:forEach>
		...
		<c:forEach var="semestre" items="${semestres }">
			<div class="main-card mb-3 card" id="${s.id_semestre}"
				style="display: none">
				<div class="card-body">
					...
					<table class="mb-0 table table-hover">
						<thead>
							<tr>
								<c:forEach var="module" items="${modules}">
									<c:if test="${module.semestre==semestre }">
										<th class="th-sm"></th>
									</c:if>
								</c:forEach>
							</tr>
						...
						<tbody>
							<tr>
								<c:forEach var="module" items="${modules}">
									<c:if test="${module.semestre==s }">
										<td><a style="color: black" name="linktobottom">${module.libelle_module}</a></td>
									</c:if>
								</c:forEach>
							</tr>
						</tbody>
