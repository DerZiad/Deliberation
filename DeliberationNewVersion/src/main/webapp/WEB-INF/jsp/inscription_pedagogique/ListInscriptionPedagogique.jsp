									...
									<c:if test="${ parModule==1}">
										<c:forEach var="m" items="${module}">
											<c:if test="${m.semestre.filiere==f && testM!=m.semestre}">
												...
												<td class="th-sm" colspan="5"
													style="text-align: center; border-left: 0.1px solid #E9ECEF; border-right: 0.5px solid #E9ECEF;"><b>${m.semestre.libelle_semestre}</b></td>
											</c:if>
										</c:forEach>
									</c:if>
								</tr>
								...
								<c:forEach var="i" items="${Inscription}">
									<tr>
										<c:if test="${f.id_filiere==i.semestre.filiere.id_filiere }">
											<td><a style="color: black">${i.etudiant.first_name_fr}
													${i.etudiant.last_name_fr}</a></td>
											<c:if test="${ parSemestre==1}">
											...
											<c:if test="${ parModule==1}">
												<c:forEach var="m" items="${module }">
													...
													<c:if
														test="${f.id_filiere==m.semestre.filiere.id_filiere }">
														<c:forEach var="im" items="${i.module}">
															<c:if test="${im.id_module==m.id_module }">
																<c:set var="condition" value="${1}" />
																<td style="border: 0.1px solid #E9ECEF;"><div
																		class="mb-2 mr-2 badge badge-primary">En cours</div></td>
															</c:if>
														</c:forEach>
														<c:if test="${condition==0 }">
															<td style="border: 0.1px solid #E9ECEF;"><div
																	class="mb-2 mr-2 badge badge-focus">Pas Encore</div></td>
														</c:if>
													</c:if>
												</c:forEach>
											</c:if>
